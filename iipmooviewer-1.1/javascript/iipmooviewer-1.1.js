...
//if(typeof jQuery == 'undefined')  window.alert("jQuery library is not found!"); 
...
var IIP = new Class({
  ...
  loadGrid: function(){
    ...
    */
	...
	//last minute change of contrast
	this.contrast = userDefContrast || 1.0;
  ...
  /*Contrast Bar, interface for the contrast bar that also calls 'refresh' on the image*/
  updateContrastBar: function(newContrast) {
	userDefContrast = newContrast;
	this.refresh()
},
  ...
  createWindows: function(){
    ...
    new Element('img', {src: 'images/iip.32x32.png', id: 'info', styles: { opacity: 0.0 } } ).injectInside('logo');
...
*/
  ...
  load: function(){
	...
	onComplete: function(transport){
	    ...
	    if(!tmp[1]) alert( "Unexpected response from server " + this.server + "which is [" + response + "].");
